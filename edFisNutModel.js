...
export function checkInnerColGroups(parentEl, areAllColGroupsSimilar = false) {
    ...
    if (parentEl instanceof HTMLElement && colGroups?.flat(1)?.length > 0) {
        ...
        for (let i = 0; i < colGroups.length; i++) {
            ...
            if (arrColGrpChilds?.every(col => col instanceof HTMLTableColElement))
                ...
                validColGroupsChildCount.push(arrColGrpChilds.filter(col => col instanceof HTMLTableColElement)
                    .length);
        ...
        Total of children: ${arrColGrpChilds?.length}`);
                ...
                arrColGrpChilds.forEach(arrColGrpChild => {
                    const childInstance = `${Object.prototype.toString.call(arrColGrpChild).slice(8, -1) ??
                        "null"}`;
                    colsInstances.push(childInstance);
                    if (childInstance !== `HTMLTableColElement`)
                        ErrorHandler.elementNotFound(arrColGrpChild, "child <col>", extLine(new Error()));
                });
            }
        }
        ...
        ErrorHandler.multipleElementsNotFound(extLine(new Error()), `arguments for checkInnerColGroups(), areColGroupValids: ${areAllColGroupsSimilar ?? false}`, parentEl, `${JSON.stringify(colGroups) || null}`);
    return [validColGroupsChildCount?.length ?? 0, areAllColGroupsSimilar];
}
export function checkTabRowsIds(tab) {
    const arrTabRowsIds = [];
    tab instanceof HTMLTableElement && tab.id === "tabDCut"
        ? Array.from(tab.querySelectorAll("tr.tabRowDCutMed")).forEach(tabRow => {
            ...
            rowId?.match(/^row/)?.toString()
                ...
                : ErrorHandler.stringError(`obtendo id da row ${tabRow}`, rowId, extLine(new Error()));
        })
        : ErrorHandler.elementNotFound(tab, "tabDC in checkTabRowsIds", extLine(new Error()));
    return arrTabRowsIds || [""];
}
export function changeTabDCutLayout(protocolo, tabDC, bodyType) {
    if ((protocolo instanceof HTMLSelectElement ||
        ...
        const filteredOpsProtocolo = Array.from(protocolo.children)?.filter(childProtocolo => childProtocolo instanceof HTMLOptionElement);
        if (filteredOpsProtocolo?.length >= Array.from(protocolo.children)?.length) {
            ...
            for (let iOp = 0; iOp < filteredOpsProtocolo.length - 1; iOp++) {
                ...
                const genderedIds = GlobalModel.filterIdsByGender(arrayTabIds, bodyType.value);
                ...
                if (arrayTabIds?.length ===
                    ...
                    if (protocolo?.value.match(/^pollock3$/i)?.toString()) {
                        if ((bodyType.value === "masculino" ||
                            ...
                            ErrorHandler.stringError("validating .value of bodyType", bodyType?.value, extLine(new Error()));
                    }
                    else if (protocolo?.value.match(/^pollock7$/i)?.toString()) {
                        ...
                        ErrorHandler.stringError("obtaining pollock value", protocolo?.value, extLine(new Error()));
                }
                else
                    console.warn(`Error verifying number and/or id from rows. Row Elements ${JSON.stringify(arrayTabIds) || null}; Obtained number: ${arrayTabIds?.length}; Expected number: ${Array.from(tabDC.rows).filter(row => row.classList.contains("tabRowDCutMed"))?.length}
            Gender-filtered elements ${JSON.stringify(genderedIds) || null}; Obtained number: ${genderedIds?.length}; Expected number: 3`);
            }
        }
        else
            console.warn(`Error checking protocol options. Total of options validated: ${filteredOpsProtocolo.length}`);
    }
    else
        ErrorHandler.multipleElementsNotFound(extLine(new Error()), "validating elements for initial execution of changeTabDCutLayout()", protocolo, tabDC, bodyType);
    return "pollock3";
}
export function defineHiddenRows(tabDC, arrayTabIds = [], genderedIds = [], context = "bin") {
    if (tabDC instanceof HTMLTableElement &&
        ...
        Array.from(tabDC.querySelectorAll("tr.tabRowDCutMed")).forEach(medTr => {
            ...
            const innerInp = medTr.querySelector("input");
            if (innerInp) {
                if (innerInp.required)
                    innerInp.removeAttribute("required");
                if (medTr?.id?.slice(-4) !== "Coxa")
                    innerInp.value = "";
            }
        });
                ...
                if (innerInp) {
                    switch (context) {
                        ...
                        case "nb":
                            ...
                            break;
                        default:
                            ErrorHandler.stringError("context", context, extLine(new Error()));
                    }
                }
            }
        });
        ...
        ErrorHandler.multipleElementsNotFound(extLine(new Error()), "argumentos para defineHiddenRows", tabDC, `${JSON.stringify(arrayTabIds) || null}`, `${JSON.stringify(genderedIds) || null}`, context);
}
//correção para limitação da fórmula de PGC
export function evaluatePGCDecay(person, targInpPGC, PGC = 0) {
    ...
    if ((person instanceof Man ||
        ...
        let decreasedPGC = decreasedPerson.calcPGC(decreasedPerson), sumAcc = 1;
        ...
        ErrorHandler.multipleElementsNotFound(extLine(new Error()), "argumentos para evaluatePGCDecay", `${JSON.stringify(person) || null}`, targInpPGC, PGC);
    if (PGC < 0 || Number.isNaN(PGC) || PGC === Math.abs(Infinity))
        PGC = 0;
    return [foundDecayPoint, PGC];
}
export function alertPGCRounding(targInpPGC) {
    if (targInpPGC instanceof HTMLInputElement) {
        ...
        spanRoundingAlertIcon instanceof HTMLSpanElement &&
            ...
            : ErrorHandler.elementNotFound(spanRoundingAlertIcon, "spanRoundingAlertIcon", extLine(new Error()));
    }
    else
        ErrorHandler.inputNotFound(targInpPGC, "targInpPGC in alertPGCRounding", extLine(new Error()));
}
