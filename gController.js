...
export function getGlobalEls(isAutocorrectOn = true, firstClick = true, context = "notNum") {
    const textConts = [
    ...
    textConts?.length > 0
        ...
        : ErrorHandler.elementNotPopulated(textConts, "textConts", extLine(new Error()));
    radioInps?.length > 0
        ? addListenerRadios(radioInps, "ed")
        : ErrorHandler.elementNotPopulated(radioInps, "radioInps", extLine(new Error()));
    dateBtns?.length > 0
        ? addListenerDateBtns(dateBtns)
        : ErrorHandler.elementNotPopulated(dateBtns, "dateBtns", extLine(new Error()));
    astDigtBtns?.length > 0
        ? addListenerAstDigitBtns(astDigtBtns)
        : ErrorHandler.elementNotPopulated(astDigtBtns, "astDigtBtns", extLine(new Error()));
    subButton instanceof HTMLButtonElement
        ? subButton.addEventListener("click", () => GlobalHandler.subForm(subButton))
        : ErrorHandler.elementNotFound(subButton, "subButton", extLine(new Error()));
    resetFormBtn instanceof HTMLButtonElement
        ? resetFormBtn.addEventListener("click", (click) => GlobalHandler.resetarFormulario(click, astDigtBtns, resetFormBtn))
        : ErrorHandler.elementNotFound(resetFormBtn, "resetFormBtn", extLine(new Error()));
    if (context === "num") {
        const numInps = Array.from(document.querySelectorAll('input[type="number"]'));
        numInps?.length > 0
            ? addListenerNumInps(numInps)
            : ErrorHandler.elementNotPopulated(numInps, "numInps", extLine(new Error()));
    }
    deactAutocorrectBtns?.length > 0
        ? (isAutocorrectOn = addListenerAutocorrectBtns(deactAutocorrectBtns, isAutocorrectOn))
        : ErrorHandler.elementNotPopulated(deactAutocorrectBtns, "deactAutoCorrectBtns", extLine(new Error()));
            ...
            if (textCont?.classList.contains("autocorrect")) {
                textCont instanceof HTMLTextAreaElement ||
                    ...
                    ? textCont.addEventListener("input", () => {
                        isAutocorrectOn = GlobalModel.checkAutoCorrect(document.querySelector('button[id^="deactAutocorrectBtn"]') ||
                            document.querySelector('input[id^="deactAutocorrectBtn"]'));
                        GlobalModel.autoCapitalizeInputs(textCont, isAutocorrectOn);
                    })
                    : ErrorHandler.inputNotFound(textCont, `target textCont id ${JSON.stringify(textCont?.id || "UNIDENTIFIED TEXTCONT")}`, extLine(new Error()));
            }
        });
    }
    else
    ...
    if (numInps.every(el => el instanceof HTMLElement)) {
        numInps.forEach(numInp => {
            numInp instanceof HTMLInputElement && numInp.type === "number"
                ...
                : ErrorHandler.inputNotFound(numInp, `target numInp id ${JSON.stringify(numInp?.id || "UNIDENTIFIED TEXTCONT")}`, extLine(new Error()));
        });
    }
    else
        ...
        radioInps.forEach(radio => {
            if (radio instanceof HTMLInputElement && radio.type === "radio") {
                radio.addEventListener("keydown", (keydown) => {
                    GlobalHandler.opRadioHandler(keydown, Array.from(document.querySelectorAll('input[id$="Yes"], input[id$="No"]' //acessando como par
                    )));
                });
                radio.addEventListener("dblclick", () => GlobalHandler.doubleClickHandler(radio));
                if (context === "ed" || context === "ag") {
                    radio.addEventListener("change", (change) => GlobalHandler.cpbInpHandler(change, radio));
                    radio.addEventListener("keydown", (keydown) => GlobalHandler.cpbInpHandler(keydown, radio));
                    context === "ag" &&
                        radio.addEventListener("change", () => GlobalHandler.deactTextInput(document.querySelectorAll('input[type="number"][id$=NumId]'), document.querySelectorAll("input[id$=NullId]")));
                }
                // radio.addEventListener("touchstart", GlobalHandler.touchStartHandler);
            }
            else
                ErrorHandler.inputNotFound(radio, `target radio id ${radio?.id || "UNDEFINED ID RADIO"}`, extLine(new Error()));
        });
    }
    else
    ...
    if (dateBtns.every(el => el instanceof HTMLElement)) {
        dateBtns.forEach(dateBtn => {
            dateBtn instanceof HTMLButtonElement
                ...
                : ErrorHandler.elementNotFound(dateBtn, `target dateBtn id ${dateBtn?.id || "UNDEFINED ID DATEBTN"}`, extLine(new Error()));
        });
    }
    else
    ...
    if (GlobalModel.checkAllGenConts(genElement, genBirthRel, genTrans, genFisAlin) &&
        ...
        arrGenConts.forEach(genCont => {
            genCont.addEventListener("change", () => {
                console.log("em callback de genconts " + genValue);
                genValue =
...
export function addListenerCite(editableCite, isAutocorrectOn = true, firstClick = true) {
    if (editableCite instanceof HTMLElement) {
        ...
        editableCite.addEventListener("keyup", (keypress) => {
            keypress.target instanceof HTMLElement
                ...
                : ErrorHandler.elementNotFound(keypress.target, "keypress with editableCite as a target", extLine(new Error()));
        });
...
export function addListenerAutocorrectBtns(deactAutocorrectBtns, isAutocorrectOn = true) {
    if (deactAutocorrectBtns.every(el => el instanceof HTMLElement)) {
        deactAutocorrectBtns.forEach(deactAutocorrectBtn => {
            deactAutocorrectBtn instanceof HTMLButtonElement ||
                ...
                : ErrorHandler.elementNotPopulated(deactAutocorrectBtns, `target deactAutocorrectBtn id ${deactAutocorrectBtn?.id || "UNDEFINED ID BUTTON"}`, extLine(new Error()));
        });
    }
    else
    ...
    if (astDigtBtns.every(el => el instanceof HTMLElement)) {
        astDigtBtns.forEach(astDigtBtn => {
            astDigtBtn instanceof HTMLButtonElement
                ...
                : ErrorHandler.elementNotFound(astDigtBtn, astDigtBtn?.id || "UNDEFINED ID BUTTON", extLine(new Error()));
        });
    }
    else
        console.error(`Erro validando instâncias em astDigtBtns`);
}
export function addListenerExportBtn(context = "undefined") {
    ...
    if (btnExport instanceof HTMLButtonElement ||
        ...
        btnExport.addEventListener("click", async () => {
            try {
                ...
                while (arrValues.length > arrTitles.length)
                    arrTitles.push("Sem_titulo");
                while (arrTitles.length > arrValues.length)
                    arrValues.push("Nulo");
                if (arrValues.length === arrTitles.length) {
                    ...
                    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(dataJSON, undefined), "Sheet1", undefined);
                    XLSX.writeFile(wb, `data_${context}form.xlsx`, undefined);
                    console.log("Spreadsheet generated.");
                }
                else
                    throw new Error(`Error validating length of data arrays`);
            }
            catch (error) {
        ...
        ErrorHandler.elementNotFound(btnExport, "argument for addListenerExportBtn()", extLine(new Error()));
    return btnExport;
}
